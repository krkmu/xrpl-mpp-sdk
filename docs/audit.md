...
| X5 | IOU paths | ~~We never set `Paths` on Payment for IOU. Many cross-issuer or rippling-required paths fail.~~ **Done** in commit `244d804` -- `sdk/src/utils/paths.ts.resolveIouPaymentExtras()` autofills Paths + SendMax with TransferRate + slippage. Default 50 bps, tunable. Unit and devnet integration tests in commit `cc7bad0`; demo in `5bf0268`. | unit: mock ripple_path_find returning a path; assert client uses it. integration: pay an IOU through a path | Use `client.request({ command: 'ripple_path_find', ... })` when currency is IOU and source != issuer; attach `Paths` to tx |
